...
def child_env(child_id: int, env_fn: Callable, child_conn: Connection) -> None:
    ...
    env = env_fn()
    c_obs, c_rew, c_done, c_info = None, None, None, None
    while True:
        ...
        if message_type == MessageType.RESET:
            ...
            if c_done is None:
                obs, rew, done, info = env.step(content)
                if done:
                    c_obs, c_rew, c_done, c_info = obs, rew, done, info
                child_conn.send(Message(MessageType.STEP_RETURN, (obs, rew, done, info)))
            else:
                child_conn.send(Message(MessageType.STEP_RETURN, (c_obs, c_rew, c_done, c_info)))
...
class MultiProcessEnv(DoneTrackerEnv):
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Any]:
        ...
        obs, rew, done, info = zip(*content)
        done = np.stack(done)
        self.update_done_tracker(done)

        return np.stack(obs), np.stack(rew), done, info
